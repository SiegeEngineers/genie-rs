...
impl TechEffect {
    ...
    pub fn write_to<W: Write>(&self, output: &mut W) -> Result<()> {
        output.write_u8(self.effect_type)?;
...
impl Tech {
    pub fn from<R: Read>(input: &mut R) -> Result<Self> {
        ...
        let mut effects = vec![TechEffect::default(); num_effects as usize];
        for effect in effects.iter_mut() {
            *effect = TechEffect::from(input)?;
        }

        Ok(Self { name, effects })
    }
